...
const DocSearch = () => {
    return(
        <div className='documentation'>
            ...
            <p>The Search bar takes several distinct types of queries. The simplest is to query for a single specific entity in the database. One can use the search to query for a specific cell (e.g. MCF-7), compound (e.g. 17-AAG), tissue (e.g. breast) or dataset (e.g. CTRPv2). This query would then take you to the corresponding page for that instance. It is feasible to search for the synonyms of a compound name or cell names as long as they are included in one of the PharmacoDB datasets.</p>
            <p>The search bar also allows the user to specify a data type (e.g. cell, compound, dataset), where it would take the user to a page listing all instances of that data type in the database.</p>
            {/*<img width="650px" alt="Searching cell line data" src={logo} title="Searching cell"/>*/}
            <p>Queries consisting of the names of multiple datasets, with a space between dataset names, are also allowed. This query will link the user to a page describing the intersection between the datasets, with tables listing the common cells, compounds and tissue types.</p>
            <div className="center"><img src={intersectionSearch}/></div>
            ...
            <div className="center"><img src={cellDrugSearch}/></div>
            <h6>Compound vs. Tissue/ Tissue vs. Compoundg</h6>
            <div className="center"><img src={drugTissueSearch}/></div>
            ...
            <div className="center"><img src={homeNav}/></div>
        </div>
    );
}
